...
class GRAVE:
    """Graph-Based Relative Average Value Estimation (GRAVE) class."""

    def __init__(self, symbol: int, t: float) -> None:
        self.symbol = symbol
        self.t = t
        self.graph: Dict[Tuple[int, int], Tuple[int, int]] = {}  # (q, n) for each action

    def compute_move(self, node: "Node") -> Tuple[int, int]:
        """Compute move using GRAVE algorithm."""
        time0 = time.time()
    ...
    def select_grave(self, node: "Node") -> "Node":
        """Select node with best GRAVE value."""
        best_value = -np.inf
        best_node = None
        for child in node.children:
            ...
            # GRAVE value
            grave_value = self.get_grave_value(child)
            # Combined value
            combined_value = uct + grave_value
            if combined_value > best_value:
                best_value = combined_value
                best_node = child
        return best_node if best_node is not None else node

    def get_grave_value(self, child: "Node") -> float:
        """Get GRAVE value for a child node."""
        action = self.get_action(child.parent.board, child.board)
        if action is None:
            return 0
        q, n = self.graph.get(action, (0, 0))
    ...
    def rollout(self, node: "Node") -> Tuple[Optional[int], List[Tuple[int, int]]]:
        """Perform a random game simulation and record actions for GRAVE."""
    ...
    def backpropagate(self, node: "Node", winner: Optional[int], actions: List[Tuple[int, int]]) -> None:
        """Update recursively node visits and scores from leaf to root, including GRAVE."""
        if node.turn == winner:
            node.q += 1
        node.n += 1
        # Update GRAVE statistics for all actions in the simulation
        for action in actions:
            q, n = self.graph.get(action, (0, 0))
            q += 1 if node.turn == winner else 0
            n += 1
            self.graph[action] = (q, n)
...
class Node:
    """Monte Carlo tree node class with GRAVE."""
